...
Quando o `/instalar` terminar, renomeia a pasta `JotaTekOS/` pro nome do teu
negócio (fecha o VS Code, renomeia no Explorer/Finder, abre de novo). A
pasta não fica como "JotaTekOS" — ela é o teu negócio agora.
...
closed loop dentro do JotaTekOS (decide → executa → captura → realimenta →
ajusta sozinho).
